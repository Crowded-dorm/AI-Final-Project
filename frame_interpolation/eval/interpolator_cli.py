...
def _output_frames(frames: List[np.ndarray], frames_dir: str):
  ...
  #end = float(frame_num//30+1)
  end = 1.0
  dir2 = f'custom_dataset/videos/PNGImages/{_PATTERN.value}_FILM_0.0-{end}_fps10'
  os.makedirs(dir2, exist_ok=True)
  if tf.io.gfile.isdir(dir2):
    old_frames = tf.io.gfile.glob(f'{frames_dir}/*.png')
    if old_frames:
      logging.info('Removing existing frames from %s.', dir2)
      for old_frame in old_frames:
        tf.io.gfile.remove(old_frame)
  else:
    tf.io.gfile.makedirs(dir2)
  for idx, frame in tqdm(
      enumerate(frames), total=len(frames), ncols=100, colour='green'):
    util.write_image(f'{dir2}/{idx:05d}.png', frame)
  logging.info('Output frames saved in %s.', dir2)
...
class ProcessDirectory(beam.DoFn):
  ...
  def process(self, directory: str):
    ...
    if _OUTPUT_VIDEO.value:
      ...
      # logging.info(f'Output video saved at {dir2}%s.mp4.', _PATTERN.value)


def _run_pipeline() -> None:
  directories = tf.io.gfile.glob('custom_dataset/frames/' + _PATTERN.value)
